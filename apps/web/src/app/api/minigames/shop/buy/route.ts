...
import { MinigameError, buyCosmetic } from '@/server/minigame-service';
...
export async function POST(request: Request) {
  ...
  try {
    return NextResponse.json(await buyCosmetic(player.id, cosmeticId));
  } catch (err) {
    if (err instanceof MinigameError) {
      return NextResponse.json({ error: err.message, code: err.code }, { status: 400 });
    }
    console.error('buy cosmetic failed', err);
